...
- Gateway startup and runtime checks now better tolerate read-only configs, missing OpenClaw state/config paths, and container health-probe requirements.
...
- Docker/runtime regressions around Compose, assets, probes, OpenClaw path resolution (including legacy `OPENCLAW_HOME` fallback), and read-only configs
