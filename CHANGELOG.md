...
- Self-build prompt preambles now include an explicit step execution response contract, performance rubric, and bounded step-iteration policy so each required loop step is scored, optimized, and verified before advancing.
...
- Added regression assertions that prompt preambles emit step response contract, performance rubric, and step iteration policy sections.
