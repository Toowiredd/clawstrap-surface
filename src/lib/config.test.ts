...
describe('config data paths', () => {
  ...
  it('derives db and token paths from MISSION_CONTROL_DATA_DIR', async () => {
    ...
    expect(toPosix(config.dataDir)).toBe('/tmp/mission-control-data')
    expect(toPosix(config.dbPath)).toBe('/tmp/mission-control-data/mission-control.db')
    expect(toPosix(config.tokensPath)).toBe('/tmp/mission-control-data/mission-control-tokens.json')
  })
  ...
  it('respects explicit db and token path overrides', async () => {
    ...
    expect(toPosix(config.dataDir)).toBe('/tmp/mission-control-data')
    expect(toPosix(config.dbPath)).toBe('/tmp/custom.db')
    expect(toPosix(config.tokensPath)).toBe('/tmp/custom-tokens.json')
  })
  ...
  it('uses a build-scoped worker data dir during next build', async () => {
    ...
    expect(toPosix(config.dataDir)).toMatch(/^\/tmp\/build-scratch\/worker-\d+$/)
    expect(toPosix(config.dbPath)).toMatch(/^\/tmp\/build-scratch\/worker-\d+\/mission-control\.db$/)
    expect(toPosix(config.tokensPath)).toMatch(/^\/tmp\/build-scratch\/worker-\d+\/mission-control-tokens\.json$/)
  })
  ...
  it('prefers build-specific db and token overrides during next build', async () => {
    ...
    const expectedBuildRoot = toPosix(path.join(os.tmpdir(), 'mission-control-build'))
    expect(toPosix(config.dataDir)).toMatch(new RegExp(`^${expectedBuildRoot.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}/worker-\\d+$`))
    expect(toPosix(config.dbPath)).toBe('/tmp/build.db')
    expect(toPosix(config.tokensPath)).toBe('/tmp/build-tokens.json')
  })
})
