...
export function OnboardingWizard() {
  const { showOnboarding, setShowOnboarding, dashboardMode, gatewayAvailable, interfaceMode, setInterfaceMode } = useClawstrap()
  ...
  return createPortal(
    <div className={`fixed inset-0 z-[140] flex items-center justify-center transition-opacity duration-300 ${closing ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Clawstrap onboarding"
...
function StepInterfaceMode({ isGateway, onNext, onBack }: {
  ...
  const tc = useTranslations('common')
  const { interfaceMode, setInterfaceMode } = useClawstrap()
